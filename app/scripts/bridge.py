...
def bridge(wallet, params, wait_balance=True):
    ...
    while True:
        try:
            ...
            bridge_data = get_bridge_data(wallet.address, srcChain.get("CHAIN_ID"), dstChain.get("CHAIN_ID"), srcTokenAddress, dstTokenAddress, amount, config.BRIDGE_SLIPPAGE)
            for i in bridge_data:
                if i["trade"]:
                    bridge_data = i
                    break

            if srcTokenAddress != config.ETH and not approve_result:
